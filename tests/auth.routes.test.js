...
import { createJsonApp, FakePool, FakeRedis, makeRepo, withServer } from './test-helpers.js';
...
test('register route validates required fields', async () => {
  ...
  const app = createJsonApp(createAuthRouter(makeRepo(pool), pool, jwtSecret, accessTokenExpiry));
...
test('forgot-password route uses frontendBaseUrl for reset links', async () => {
  ...
  await repo.createUser('user@example.com', 'hashed-password', 'Pranshul', 'user');
...
test('verify-email route verifies a valid token and rejects invalid token', async () => {
  const pool = new FakePool();
  const repo = makeRepo(pool);
  const user = await repo.createUser('user@example.com', 'hashed-password', 'Pranshul', 'user');
  const token = 'verify-token';
  await repo.setEmailVerificationToken(user.id, hashToken(token), new Date(Date.now() + 60_000));

  const app = createJsonApp(createAuthRouter(repo, pool, jwtSecret, accessTokenExpiry));
